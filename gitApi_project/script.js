...
async function fetchRepos(query) {
  try {
    ...
    autoCompleteBox.innerHTML = ''; // Очищаем автокомплит перед новым запросом
    
    // Если есть репозитории, добавляем их в автокомплит
...
// Обработчик события ввода для поиска репозиториев с использованием дебаунса
...
// Обработчик клика по автокомплиту
autoCompleteBox.addEventListener('click', (event) => {
  ...
  if (listItem) {
    ...
    // Добавляем элемент в основной список при клике
